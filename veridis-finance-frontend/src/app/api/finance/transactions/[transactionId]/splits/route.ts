...
export async function GET(
  ...
  const backendResponse = await fetch(
    backendUrl(
      `/api/finance/transactions/${context.params.transactionId}/splits${request.nextUrl.search}`
    ),
    {
      method: "GET",
      headers: {
...
export async function POST(
  ...
  const backendResponse = await fetch(
    backendUrl(`/api/finance/transactions/${context.params.transactionId}/splits`),
    {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
