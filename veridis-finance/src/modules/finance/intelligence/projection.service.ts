...
export interface CashflowProjection {
  ...
  estimated_negative_date: Date | null;
  trend: CashflowTrend;
}
...
interface RecurringTotalRow {
  type: "income" | "expense";
  monthly_total: string | number;
}

interface LookbackTotals {
  income: number;
  expenses: number;
}
...
function getLookbackStart(now: Date, lookbackDays = LOOKBACK_DAYS): Date {
  return new Date(now.getTime() - lookbackDays * 24 * 60 * 60 * 1000);
}

function getEndOfMonthUtc(now: Date): Date {
  return new Date(
    Date.UTC(
      now.getUTCFullYear(),
      now.getUTCMonth() + 1,
...
function getRemainingDays(now: Date, target: Date): number {
  const msInDay = 24 * 60 * 60 * 1000;
  const diff = target.getTime() - now.getTime();
  return Math.max(0, Math.ceil(diff / msInDay));
}

function getProjectedReceivables(pendingInvoices: number, horizonDays: number): number {
  const ratio = Math.min(1, Math.max(0, horizonDays / DAYS_IN_MONTH));
  return toMoney(pendingInvoices * ratio);
}

function getMonthlyAverages(totals: LookbackTotals): MonthlyAverages {
...
function classifyTrend(monthlyRunRateNet: number, avgMonthlyExpenses: number): CashflowTrend {
...
function estimateNegativeBalanceDate(
  startBalance: number,
  monthlyRunRateNet: number,
  now: Date
): Date | null {
  if (startBalance < 0) {
    return now;
...
async function fetchLookbackTotals(
  db: Queryable,
  organizationId: string,
  lookbackStart: Date
): Promise<LookbackTotals> {
  const query = {
    text: `
      SELECT
        type,
  ...
  const result = await db.query<GroupedAmountRow>(query);

  return result.rows.reduce<LookbackTotals>(
    (acc, row) => {
      if (row.type === "income") {
...
async function fetchCurrentBalance(db: Queryable, organizationId: string): Promise<number> {
  const query = {
    text: `
      SELECT
  ...
  const result = await db.query<BalanceRow>(query);
  const row = result.rows[0];
  return toMoney(row?.current_balance ?? 0);
}

async function fetchPendingInvoicesTotal(
  db: Queryable,
  organizationId: string
): Promise<number> {
  const query = {
  ...
  const result = await db.query<PendingInvoiceRow>(query);
  return toMoney(result.rows[0]?.pending_total ?? 0);
}

async function fetchRecurringMonthlyTotals(
  db: Queryable,
  organizationId: string,
  lookbackStart: Date
): Promise<{ recurringIncomeMonthly: number; recurringExpenseMonthly: number }> {
  const query = {
    text: `
  ...
  const result = await db.query<RecurringTotalRow>(query);
...
export async function calculateCashflowProjection(
  organizationId: string,
  dependencies: ProjectionServiceDependencies = {}
): Promise<CashflowProjection> {
  assertOrganizationId(organizationId);

  const db = dependencies.db ?? (pool as unknown as Queryable);
